...
const UnsatConc = () => {
  ...
  // 엑셀 데이터 받기
  const getData = () => {
    let preData = sessionStorage.getItem("unsat_soilconc");
  ...
  const reset = () => {
    sessionStorage.removeItem("unsat_soilconc");
  ...
  const makeJSON = (numOfRows) => {
    ...
    sessionStorage.setItem("unsat_soilconc", result);
  };

  // json 새 창
  const openData = () => {
    window.open("/unsatsoilconc", "", "_blank");
  };
