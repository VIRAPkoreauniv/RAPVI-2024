...
const SatDeffi = () => {
  ...
  // 엑셀 데이터 받기
  const getData = () => {
    let preData = sessionStorage.getItem("DeffT");
  ...
  const reset = () => {
    sessionStorage.removeItem("DeffT");
  ...
  const makeJSON = (numOfRows) => {
    ...
    sessionStorage.setItem("DeffT", result);
  };

  // json 새 창
  const openData = () => {
    window.open("/deffi", "", "_blank");
  };
